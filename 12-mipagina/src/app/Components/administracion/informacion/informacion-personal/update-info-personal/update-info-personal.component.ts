...
export class UpdateInfoPersonalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.suscripcionId = this.activatedRoute.params.subscribe(params => {
      this.infoPerService.getdocumentInfoPersonal(params.id).subscribe(document => {
        ...
        this.informacionPersonalInicial = {...this.informacionPersonal} // clonar
      }, error => {
        Swal.fire({
          position: 'top-end',
          icon: 'error',
  ...
  async actualizarDatosmodal(forma: any) {
    let resp
    if (JSON.stringify(this.informacionPersonal) === JSON.stringify(this.informacionPersonalInicial)) {
      Swal.fire({
        position: 'top-end',
        icon: 'warning',
        title: 'No se puede actualizar',
        text: 'Los datos no han cambiado',
      })
    } else {
